accelerate launch train_second_stage.py \
  --pretrained_model_name_or_path="path_to_sdxl/stable-diffusion-xl-base-1.0/stable-diffusion-xl-base-1.0"  \
  --instance_data_dir="path_to_style" \
  --instance_data_dir_2="path_to_content" \
  --pretrained_vae_model_name_or_path="path_to_vae/stable-diffusion-xl-base-1.0/sdxl-vae-fp16-fix" \
  --output_dir="output_path" \
  --instance_prompt="Prompt to describe the style. e.g. A woman walking a dog in w@z flat cartoon illustration style" \
  --instance_prompt_2="Prompt to describe the content. e.g. A photo of snq teddybear" \
  --resolution=512 \
  --rank=64 \
  --train_batch_size=1 \
  --gradient_accumulation_steps=4 \
  --learning_rate=1e-4 \
  --learning_rate_2=1e-4 \
  --report_to="tensorboard" \
  --lr_scheduler="constant" \
  --lr_warmup_steps=0 \
  --max_train_steps=1000 \
  --seed="0" \
  --checkpointing_steps=50 \
  --exchange_finetune \
  --lora_path_obj="path_to_content_lora/pytorch_lora_weights.safetensors" \
  --lora_path_style="path_to_style_lora/pytorch_lora_weights.safetensors"